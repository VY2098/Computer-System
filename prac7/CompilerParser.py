...
class CompilerParser :
    ...
    def compileSubroutineBody(self):
        ...
        while not self.have("symbol", "}"):
            ...
            elif self.current().getType() == "keyword" and self.current().getValue() in ["let", "if", "while", "do", "return"]:
    ...
    def compileExpression(self):
        ...
        while self.current() and self.current().getType() == "symbol" and self.current().getValue() in op:
            expression_tree.addChild(self.mustBe("symbol", self.current().getValue()))
            expression_tree.addChild(self.compileTerm())
    ...
    def compileTerm(self):
        ...
        if self.current().getType() == "integerConstant":
            term_tree.addChild(self.mustBe("integerConstant", self.current().getValue())) 

        # string constant
        elif self.current().getType() == "stringConstant":
        ...
        # unary operation
            ...
            term_tree.addChild(self.mustBe("symbol", self.current().getValue()))
            term_tree.addChild(self.compileTerm())

        # identifier
        elif self.current().getType() == "identifier":
            term_tree.addChild(self.mustBe("identifier", self.current().getValue()))
            if self.have("symbol", "["):
    ...
    def compileExpressionList(self):
        ...
        """
        expr_list_tree = ParseTree("expressionList", "")

        if self.current().getType() == "symbol" and self.current().getValue() == ")":
            return expr_list_tree

        if self.current() is None:
            return expr_list_tree

        expr_list_tree.addChild(self.compileExpression())

        # While there are more expressions separated by commas, keep adding them
        while self.have("symbol", ","):
            expr_list_tree.addChild(self.mustBe("symbol", ","))
            expr_list_tree.addChild(self.compileExpression())
